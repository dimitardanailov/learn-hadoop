...
for line in sys.stdin:
	line = line.strip() # string out carriage return
	key_value = line.split('\t') # split line, into key and value, returns a list
	line_cnt = line_cnt + 1

	# note: for simple debuggin use print statements, ie:
	...
	if curr_word != prev_word:
		...
		if line_cnt > 1:
			...
			dates_to_output = []
			day_cnts_to_output = []
		prev_word = curr_word # set up previous word for the next set of input files
	
	# ---------------------------------------------------------------
	# whether or not the join result was written out, now process the curr word    
	...
	if (value_in[0:3] in months):
		date_day = value_in.split() # split the value field into a date and day_cnt

		# Addd date to list of the fields we are building
		dates_to_output.append(date_day[0])
		day_cnts_to_output.append(date_day[0])
		day_cnts_to_output.append(date_day[1])
	else:
		curr_word_total_cnt = value_in # if the value field was just the total count the its 
